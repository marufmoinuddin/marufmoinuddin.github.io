---
layout: post
title: pgBackRest setup
date: 2024-12-02
category: PostgreSQL
tags: [backup, pgbackrest, postgresql, s3, ubuntu]
excerpt: "pgBackRest is a backup tool used to perform PostgreSQL database backup, archiving, restoration, and point-in-time recovery. While it can be used for local backups, this procedure shows how to deploy a pgBackRest server…"
read_time: 6
source_doc: pg-docs/pgbackrest.md
draft_import: true
---
# pgBackRest setup
...
You also need a backup storage to store the backups. It can either be a remote storage such as AWS S3, S3-compatible storages or Azure blob storage, or a filesystem-based one. For our demonestration we will create a virtual machine like the previous tutorial.

## Create a VM as backup server
Please refer to previous tutorial if you need to install Virt-Manager and setup Ubuntu server. For this tutorial I will consider that you already have completed setting up the VM. Also please consider increasing or decreasing storage size if needed.


## Configure backup server

Like the previous tutorial please ssh to your VM for convenience.
...
9. If you are using Patroni, change its configuration to use `pgBackRest` for archiving and restoring WAL files. Run this command only on one node, for example, on `node1`: 
	...
	postgresql:
	  parameters:
	    # Add archive_mode parameter here
	    archive_mode: on
